...
# test_board = Board([[1,3,4,0],[0,0,4,3]], [4,1])
...
def minimax_max_limit_caching(board, curr_player, heuristic_func, depth_limit, cache):
    ...
    if (curr_player, board) in cache:
        c_move, c_value, depth = cache[(curr_player, board)]
    ...
    for move in moves:
        ...
        _, value = minimax_min_limit_caching(next_board, opponent, heuristic_func, depth_limit-1, cache)

        if value > best_value:
            best_move = move
            best_value = value
    
    cache[(curr_player, board)] = best_move, best_value, depth_limit
    return best_move, best_value
    # raise NotImplementedError
...
def minimax_min_limit_caching(board, curr_player, heuristic_func, depth_limit, cache):
    ...
    for move in moves:
        next_board = play_move(board, curr_player, move)
        _, value = minimax_max_limit_caching(next_board, opponent, heuristic_func, depth_limit-1, cache)
